...
const { modelProvider: { User, Client, AccessToken, RefreshToken } } = require('app/app');
...
class AuthDelegate {
  ...
  findUser(context) {
    ...
    if (id) {
      ...
      return User
        .findOne({ username: login.toLowerCase(), suspendedAt: { $exists: false } })
        .then((user) => {
  ...
  findUserByToken(context) {
    return Bb.try(() => {
      ...
      .then((token) => {
        if (token) {
          return User
            .findOne({ _id: token.userId, suspendedAt: { $exists: false } })
            .then((user) => {
              if (!user) {
    ...
    return Bb
      .try(() => {
        if (user) {
          ...
          return User.findOne({ _id: authorizationCode.userId, suspendedAt: { $exists: false } });
        }
      })
      .then((user) => {
        if (!user) {
